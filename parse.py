...
tmadelon_trgX, tmadelon_tstX, tmadelon_trgY, tmadelon_tstY = ms.train_test_split(madX, madY, test_size=0.3, random_state=0,stratify=madY)
madelon_trgX, madelon_tstX, madelon_trgY, madelon_tstY = ms.train_test_split(tmadelon_tstX, tmadelon_tstY, test_size=0.3, random_state=0,stratify=tmadelon_tstY)
...
cols[-1] = 'Class'
digits.columns = cols
digitsX = pd.DataFrame(digitsX)
digitsY = pd.DataFrame(digitsY)
digitsY.columns = ['Class']

digits_trgX, digits_tstX, digits_trgY, digits_tstY = ms.train_test_split(digitsX, digitsY, test_size=0.2, random_state=0,stratify=digitsY)

digitsX = pd.DataFrame(digits_tstX)
digitsY = pd.DataFrame(digits_tstY)
digitsY.columns = ['Class']

print(digitsY.shape)

digits = pd.concat([digitsX,digitsY],1)
digits = digits.dropna(axis=1,how='all')
